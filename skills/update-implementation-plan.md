...
## When to Use
Use this skill when the user wants to propagate approved specification updates into execution planning, add new Slice Plans required by approved updates, or adjust `draft` or `approved` Slice Plans before the affected work has started.
...
- “add slice plans for the approved update”
...
- inspect current plan status and Slice Plan state
- identify whether the affected work is still `draft`, already `approved`, or already `done`
...
- add new Slice Plans if new work is required
- adjust existing `draft` or `approved` Slice Plans when their scope or ordering must change before execution starts
- keep the execution model small, explicit, and reviewable

Respect Shape’s execution boundaries:
- `done` Slice Plans should not be rewritten
- `approved` Slice Plans with completed tasks should not be reshaped in ways that invalidate active execution
...
- prefer adding new Slice Plans or notes that account for the changed slice direction
...
- YOU MUST NOT apply accepted execution-shape changes, new Slice Plans, or revised Slice Plan descriptions as final unless the Developer explicitly approves them.
...
- YOU MUST NOT proceed to Slice Plan approval, batch implementation, or any other next Shape workflow step unless the Developer explicitly approves proceeding.
...
- only `draft` Slice Plans or `approved` Slice Plans without completed tasks may be updated
- `approved` Slice Plans with completed tasks, `done` Slice Plans, and any Implementation Tasks must not be rewritten by this skill
- `## Relevant Files` may be refreshed when upcoming execution areas changed

This skill may:
- append new Slice Plans
- update existing `draft` or not-yet-started `approved` Slice Plan descriptions
...
- mark Slice Plans or tasks done
- rewrite `done` or already-started `approved` Slice Plans as if no work has happened
- treat draft upstream updates as effective

## Outputs
This skill should produce:
- an updated Implementation Plan aligned to approved upstream changes
- any new or revised `draft` or not-yet-started `approved` Slice Plans needed for execution
- surfaced warnings when propagation is constrained by already-started or `done` work
...
- any new work is visible through Slice Plans rather than implied in prose alone
- protected boundaries around already-started work, `done` work, and tasks were preserved
...
- Do not silently reinterpret closed or already-started Slice Plans
- Do not use `## Relevant Files` as a substitute for actual slice or Slice Plan updates when execution shape changed
...
## Likely Next Step
Usually suggest:
- `plan slice` if the updated plan now has a clear next `draft` Slice Plan
...
Prefer `plan slice` once the updated plan has a clear next `draft` Slice Plan to refine, the commit boundary is resolved, and the Developer explicitly approves proceeding.
