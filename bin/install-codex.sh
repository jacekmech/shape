...
usage() {
    cat <<'EOF'
Usage:
  install-codex.sh [--overwrite] <shape-root> <target-root>
...
Examples:
  ...
  install-codex.sh --overwrite . ../demo-repo
EOF
}
...
This repository uses the Shape workflow for AI-assisted software development.
...
- Treat feature folders under the configured feature root as the primary Shape work units.
